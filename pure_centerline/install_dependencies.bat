...
rem  required by Pure_Centerline.pyt inside ArcGIS Pro.
rem
rem  PACKAGES REQUIRED
rem  -----------------
rem  networkx  — the only package that needs to be installed.
rem  numpy and scipy ship with every ArcGIS Pro Python environment.
rem
rem  HOW TO RUN
rem  ----------
rem  Option A (recommended):
rem    Start > ArcGIS > ArcGIS Pro Python Command Prompt
rem    cd to the pure_centerline folder and run:
rem      install_dependencies.bat
rem
rem  Option B:
...
echo ============================================================
echo  Pure Centerline -- Dependency Installer
echo ============================================================
echo.
echo This toolbox needs only ONE third-party package: networkx
echo (numpy and scipy are already present in ArcGIS Pro).
echo.
...
rem ---- conda install -----------------------------------------------------
echo Installing networkx via conda (conda-forge channel) ...
echo.
...
if errorlevel 1 (
    ...
    echo   conda create --name arcgispro-py3-pure --clone arcgispro-py3
    echo   activate arcgispro-py3-pure
    echo   install_dependencies.bat
    echo.
    echo Then in ArcGIS Pro:
    echo   Project ^> Python ^> Python Environments ^> arcgispro-py3-pure
    echo.
    goto :pip_fallback
)
...
echo  You can now run the Pure Centerline toolbox in ArcGIS Pro.
echo ============================================================
echo.
pause
exit /b 0
...
echo  For full instructions see: pure_centerline/README.md
echo ============================================================
echo.
pause
exit /b 1
